    ...
    <div class="modal-dialog modal-md" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="template-modal-label"><span id="template-modal-title"></span>
                    {$lang.template}</h4>
            </div>
                                ...
                                <div class="panel-heading" role="tab" id="headingTwo">
                                    <h4 class="panel-title">
                                        <a class="collapsed" role="button" data-toggle="collapse"
                                            ...
                                            {AdminLang::trans('setup.products')} <small>(click to expand)</small>
                                        </a>
                                    </h4>
                                </div>
                                    ...
                                    <div class="panel-body">
                                        ...
                                        {if count($products) gt 0}
                                            {foreach $products as $product}
                                                <label>
                                                    <input type="checkbox" name="template-product[{$product->id}]"
                                                        value="{$product->id}" />
                                                    {$product->name}
                                                </label>
                                                <br />
                                            {/foreach}
                                        {else}
                                            <i>There are currently no products available.</i>
                                        {/if}
                                    </div>
                                </div>
                            </div>
                            <div class="panel panel-info">
                                <div class="panel-heading" role="tab" id="headingOne">
                                    <h4 class="panel-title">
                                        <a role="button" data-toggle="collapse" data-parent="#accordion"
                                            ...
                                            {$lang.instructions} <small>(click to expand)</small>
                                        </a>
                                    </h4>
                                </div>
                                <div id="collapseOne" class="panel-collapse collapse" role="tabpanel"
                                    ...
                                    <div class="panel-body">
                                        ...
                                        <ul>
                                            <li>{$lang.instructions3}:<br />A, AAAA, MX, MXE, CNAME, TXT, SRV, URL,
                                                FRAME, NS</li>
                                            <li>{$lang.instructions4}</li>
                                            <li>{$lang.instructions5}</li>
                                            <li>{$lang.instructions6}</li>
                                            <li>{$lang.instructions7}</li>
                                        </ul>
                                        {$lang.example}:
                                        <pre>@ A %ip%<br />www CNAME @<br />mail A 127.0.0.1<br />@ MX mail.@ 10<br />hostname 3600 IN NS ns1.mydomain.com.</pre>
                                    </div>
                                </div>
                            </div>
                        </div>
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-primary" id="btn-edit"><i class="fas fa-save"
                        id="template-icon"></i> {AdminLang::trans('global.save')}</button>
                <button type="button" class="btn btn-danger" id="btn-delete"><i class="fas fa-trash"
                        id="delete-icon"></i> {AdminLang::trans('global.delete')}</button>
                <button type="button" class="btn btn-default" data-dismiss="modal"><i class="fas fa-times-circle"></i>
                    {AdminLang::trans('global.close')}</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    $(document).ready(function() {
            ...
            if (templateId) {
                $.ajax({
                    ...
                    for (const template of data) {
                        $("input:checkbox[name='template-product[" + template.product_id +
                            "]']").prop('checked', true);
                    }
                    $('#btn-delete').show();
                }).fail(function(xhr) {
        ...
        $('#btn-edit').click(function() {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    products: $("input:checkbox:checked[name^='template-product']").map(
                        function() {
                            return $(this).val();
                        }).get()
                }
        ...
        $('#btn-delete').click(function() {
            ...
            $('#delete-icon').removeClass('fa-trash').addClass('fa-spinner fa-spin'); $('#btn-delete').prop(
                'disabled', true); $.ajax({
                url: '{$modulelink}&page=service',
                type: 'POST',
                dataType: 'json',
