...
<script>
    $(document).ready(function() {
        $(".download-contents").click(function() {
            let textArea = $(this).data("download-target");
            let fileName = $(this).data("download-name");
            let downloadLink = document.createElement("a");
            downloadLink.href = "data:text/plain;charset=utf-8," + encodeURIComponent($(textArea)
            .text());
    ...
    {if $cert}
        ...
        {if $logo}
            <div class="row py-2">
                ...
                <div class="col-md-8"><img
                        src="{$webRoot}/modules/addons/cnicssl_addon/logos/{$logo}?ts={constant("CNIC_VERSION")}" /></div>
            </div>
        {/if}
        <div class="row py-2">
        ...
        {if $cert.validation == "EMAIL"}
            <div class="row py-2">
                ...
                <div class="col-md-8">
                    ...
                    {if in_array($cert.status|strtoupper, ['PROCESSING', 'REQUESTED', 'REQUESTEDCREATE', 'REQUESTEDRENEW', 'REQUESTEDREISSUE', 'PENDING', 'PENDINGCREATE', 'PENDINGRENEW', 'PENDINGREISSUE'])}
                        <br /><br />
                        <form method="post" action="{$smarty.server.PHP_SELF}?action=productdetails&amp;id={$id}">
                            <button type="submit" class="btn btn-info"
                                name="sslresendcertapproveremail">{$LANG.sslresendcertapproveremail}</button>
                        </form>
                    {/if}
                </div>
            </div>
        {elseif $cert.validation == "URL"}
            <div class="row py-2 dcv-property">
                <div class="col-md-4 dcv-field">File name</div>
                <div class="col-md-8 dcv-value">
                    <div class="input-group">
                        <input type="text" class="form-control" id="dcv-file-name" value="{$cert.validationurl}" readonly="">
                        <div class="input-group-btn input-group-append">
                            <button type="button" class="btn btn-default copy-to-clipboard"
                                    data-clipboard-target="#dcv-file-name">
            ...
            <div class="row py-2 dcv-property">
                ...
                <div class="col-md-8 dcv-value">
                    <div class="input-group">
                        <textarea class="form-control" id="dcv-file-contents" readonly="">{$cert.validationurlcontent}</textarea>
                        <button type="button" class="btn btn-default copy-to-clipboard"
                                data-clipboard-target="#dcv-file-contents">
                            <img src="{$webRoot}/assets/img/clippy.svg" alt="Copy to clipboard" width="15">
                        </button>
                    </div>
                </div>
            </div>
        {elseif $cert.validation == "FILE"}
            {foreach $cert.fileauthname as $key => $fileauthname}
                <div class="row py-2 dcv-property">
                    <div class="col-md-4 dcv-field">File name</div>
                    <div class="col-md-8 dcv-value">
                        <div class="input-group">
                            <input type="text" class="form-control" id="dcv-file-name-{$key}"
                                   value="{$cert.fileauthname[$key]}"
                                   readonly="">
                            <div class="input-group-btn input-group-append">
                                <button type="button" class="btn btn-default copy-to-clipboard"
                                        data-clipboard-target="#dcv-file-name-{$key}">
                                    <img src="{$webRoot}/assets/img/clippy.svg" alt="Copy to clipboard" width="15">
                                </button>
                            </div>
                        </div>
                ...
                <div class="row py-2 dcv-property">
                    <div class="col-md-4 dcv-field">File contents</div>
                    <div class="col-md-8 dcv-value">
                        <textarea class="form-control" id="dcv-file-contents-{$key}" readonly="">{$cert.fileauthcontents[$key]}</textarea>
                        <button type="button" class="btn btn-default copy-to-clipboard"
                                data-clipboard-target="#dcv-file-contents-{$key}">
                            <img src="{$webRoot}/assets/img/clippy.svg" alt="Copy to clipboard" width="15">
                        </button>
                    </div>
                </div>
            {/foreach}
        {elseif $cert.validation == "DNS" || $cert.validation == "DNSZONE"}
            {foreach $cert.dnstype as $key => $val}
                <div class="row py-2 dcv-property">
                    <div class="col-md-4 dcv-field">{$LANG.ssl.type}</div>
                    <div class="col-md-8 dcv-value">{$cert.dnstype[$key]}</div>
                </div>
                <div class="row py-2 dcv-property">
                    <div class="col-md-4 dcv-field">{$LANG.ssl.host}</div>
                    <div class="col-md-8 dcv-value">
                        <div class="input-group">
                            <input type="text" class="form-control" id="dcv-dns-host-{$key}" value="{$cert.dnshost[$key]}" readonly="">
                            <div class="input-group-btn input-group-append">
                                <button type="button" class="btn btn-default copy-to-clipboard"
                                        data-clipboard-target="#dcv-dns-host-{$key}">
                                    <img src="{$webRoot}/assets/img/clippy.svg" alt="Copy to clipboard" width="15">
                                </button>
                            </div>
                        </div>
                ...
                <div class="row py-2 dcv-property">
                    ...
                    <div class="col-md-8 dcv-value">
                        <div class="input-group">
                            <input type="text" class="form-control" id="dcv-dns-value-{$key}" value="{$cert.dnsvalue[$key]}" readonly="">
                            <div class="input-group-btn input-group-append">
                                <button type="button" class="btn btn-default copy-to-clipboard"
                                        data-clipboard-target="#dcv-dns-value-{$key}">
                                    <img src="{$webRoot}/assets/img/clippy.svg" alt="Copy to clipboard" width="15">
                                </button>
                            </div>
                        </div>
        ...
        <ul class="nav nav-tabs" id="contactTab" role="tablist">
            <li class="nav-item" role="presentation">
                <a class="nav-link active" id="owner-tab" data-toggle="tab" href="#owner" role="tab" aria-controls="crt"
                    aria-selected="true">
                    {$LANG.owner}
                </a>
            </li>
            <li class="nav-item" role="presentation">
                <a class="nav-link" id="admin-tab" data-toggle="tab" href="#admin" role="tab" aria-controls="ca"
                    aria-selected="false">
                    {$LANG.adminContact}
                </a>
            </li>
            <li class="nav-item" role="presentation">
                <a class="nav-link" id="tech-tab" data-toggle="tab" href="#tech" role="tab" aria-controls="csr"
                    aria-selected="false">
                    {$LANG.techContact}
                </a>
            </li>
            <li class="nav-item" role="presentation">
                <a class="nav-link" id="billing-tab" data-toggle="tab" href="#billing" role="tab" aria-controls="csr"
                    aria-selected="false" }>
                    {$LANG.billingContact}
                </a>
            </li>
        ...
        <ul class="nav nav-tabs" id="certTab" role="tablist">
            <li class="nav-item" role="presentation">
                <a class="nav-link active" id="csr-tab" data-toggle="tab" href="#csr" role="tab" aria-controls="csr"
                    aria-selected="true" {if !$cert.csr} disabled{/if}>
                    {$LANG.sslcsr}
                </a>
            </li>
            {if !in_array($cert.status, ['REQUESTED', 'REQUESTEDCREATE', 'PENDING', 'PENDINGCREATE'])}
                <li class="nav-item" role="presentation">
                    <a class="nav-link" id="crt-tab" data-toggle="tab" href="#crt" role="tab" aria-controls="crt"
                        aria-selected="false" {if !$cert.crt} disabled{/if}>
                        {$LANG.sslcrt}
                    </a>
                </li>
                <li class="nav-item" role="presentation">
                    <a class="nav-link" id="ca-tab" data-toggle="tab" href="#ca" role="tab" aria-controls="ca"
                        aria-selected="false" {if !$cert.cacrt} disabled{/if}>
                        {$LANG.sslcacrt}
                    </a>
                </li>
                <li class="nav-item" role="presentation">
                    <a class="nav-link" id="root-tab" data-toggle="tab" href="#root" role="tab" aria-controls="root"
                        aria-selected="false" {if !$cert.rootcrt} disabled{/if}>
                        {$LANG.rootcrt}
                    </a>
                </li>
            {/if}
        ...
        <div class="tab-content" id="certTabContent">
            <div class="tab-pane fade show active mt-2" id="csr" role="tabpanel" aria-labelledby="csr-tab">
                ...
                <div class="btn-group" role="group" aria-label="...">
                    <button type="button" class="btn btn-default btn-sm copy-to-clipboard pull-right float-right"
                        data-clipboard-target="#txt-csr">
                        <i aria-hidden="true" class="far fa-clipboard-list fa-lg" title="{lang key='copyToClipboard'}"></i>
                        {lang key='copyToClipboard'}
                    </button>
                    <button type="button" class="btn btn-default btn-sm download-contents pull-right float-right"
                        data-download-target="#txt-csr" data-download-name="csr.pem">
                        <i aria-hidden="true" class="far fa-file-download fa-lg" title="{lang key='downloadname'}"></i>
            ...
            {if !in_array($cert.status, ['REQUESTED', 'REQUESTEDCREATE', 'PENDING', 'PENDINGCREATE'])}
                <div class="tab-pane fade mt-2" id="crt" role="tabpanel" aria-labelledby="crt-tab">
                    ...
                    <div class="btn-group" role="group" aria-label="...">
                        <button type="button" class="btn btn-default btn-sm copy-to-clipboard pull-right float-right"
                            data-clipboard-target="#txt-crt">
                            <i aria-hidden="true" class="far fa-clipboard-list fa-lg" title="{lang key='copyToClipboard'}"></i>
                            {lang key='copyToClipboard'}
                        </button>
                        <button type="button" class="btn btn-default btn-sm download-contents pull-right float-right"
                            data-download-target="#txt-crt" data-download-name="cert.pem">
                            <i aria-hidden="true" class="far fa-file-download fa-lg" title="{lang key='downloadname'}"></i>
                            {lang key='downloadname'}
                        </button>
                    </div>
                ...
                <div class="tab-pane fade mt-2" id="ca" role="tabpanel" aria-labelledby="ca-tab">
                    ...
                    <div class="btn-group" role="group" aria-label="...">
                        <button type="button" class="btn btn-default btn-sm copy-to-clipboard pull-right float-right"
                            data-clipboard-target="#txt-cacrt">
                            <i aria-hidden="true" class="far fa-clipboard-list fa-lg" title="{lang key='copyToClipboard'}"></i>
                            {lang key='copyToClipboard'}
                        </button>
                        <button type="button" class="btn btn-default btn-sm download-contents pull-right float-right"
                            data-download-target="#txt-cacrt" data-download-name="ca.pem">
                            <i aria-hidden="true" class="far fa-file-download fa-lg" title="{lang key='downloadname'}"></i>
                            {lang key='downloadname'}
                        </button>
                    </div>
                ...
                <div class="tab-pane fade mt-2" id="root" role="tabpanel" aria-labelledby="root-tab">
                    <textarea rows="15" class="form-control" id="txt-rootcrt" readonly="">{$cert.rootcrt}</textarea>
                    <div class="btn-group" role="group" aria-label="...">
                        <button type="button" class="btn btn-default btn-sm copy-to-clipboard pull-right float-right"
                            data-clipboard-target="#txt-rootcrt">
                            <i aria-hidden="true" class="far fa-clipboard-list fa-lg" title="{lang key='copyToClipboard'}"></i>
                            {lang key='copyToClipboard'}
                        </button>
                        <button type="button" class="btn btn-default btn-sm download-contents pull-right float-right"
                            data-download-target="#txt-rootcrt" data-download-name="root.pem">
                            <i aria-hidden="true" class="far fa-file-download fa-lg" title="{lang key='downloadname'}"></i>
                            {lang key='downloadname'}
                        </button>
                    </div>
