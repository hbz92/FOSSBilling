  ...
  mainWidget() {
    ...
    if (
      ...
      $(`#panel${widgetId} .widget-tools`).prepend(
        ` <a href = "#" class="cnr-widget-toggle" data-status="${widgetStatus}">
    ...
    if (!$(this.timerSelector).length) {
      $(`#panel${widgetId} .widget-tools`).prepend(
        ` <a href="#" class="cnr-widget-expires" data-expires="${expires}" data-ttl="${ttl}">
            <span id="cnrbalexpires${widgetId}" class="ttlcounter"> </span>
          </a>`
      );
    }
      ...
      .on(
        ...
        function (event) {
          ...
          this.cnrRefreshWidget(
            widget,
            "refresh=1&status=" + newstatus+"&token=" + (csrfToken ?? WHNCS?.csrfToken ?? ""),
            function () {
              icon.attr(
                "class",
